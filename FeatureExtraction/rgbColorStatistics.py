...
def eval_image_set(image_set):
    set_size = len(image_set)
    stats_table = list()
    for i, image in enumerate(image_set):
        stats_table.append(whole_image_statistics(image))
    ...
    for s in zip(*stats_table):
        print(*s)
    plt.show()


def whole_image_statistics(image):
    b, g, r = image[:, :, 0], image[:, :, 1], image[:, :, 2]

    # blue channel
    mean_b = np.mean(b)
    variance_b = np.var(b)
    skew_b = skew(b, axis=None)

    # green channel
    mean_g = np.mean(g)
    variance_g = np.var(g)
    skew_g = skew(g, axis=None)

    # red channel
    mean_r = np.mean(r)
    variance_r = np.var(r)
    skew_r = skew(r, axis=None)
