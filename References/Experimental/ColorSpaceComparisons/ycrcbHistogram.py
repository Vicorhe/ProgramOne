...
def get_image_set():
    ...
    set_param = args['set']
    image_set = list()
    for path in sorted(glob('/Users/victorhe/Pictures/processedTileGroups/%s/*.BMP' % set_param)):
        img = cv.imread(path)
...
def plot_image_set(image_set):
    ...
    for i, (image, name) in enumerate(image_set):
        rgb = cv.cvtColor(image, cv.COLOR_YCrCb2RGB)

        plt.subplot(set_size, 2, 2*i+1), plt.imshow(rgb)
        plt.title(name), plt.xticks([]), plt.yticks([])

        plt.subplot(set_size, 2, 2*i+2), plot_ycrcb_histogram(image)
        plt.title('hist_' + str(i + 1)), plt.xticks([]), plt.yticks([])

    plt.show()


def plot_ycrcb_histogram(image):
    color = ('k', 'c', 'y')
